...
public class BlockDrawbridge extends BlockBase implements ITileEntityProvider{
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if(!worldIn.isRemote) {
			playerIn.openGui(Main.instance, Reference.GUI_DRAWBRIDGE, worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
		
		if(playerIn.isSneaking()) {
			worldIn.setBlockState(pos.north(2).down(50), BlockInit.SINKHOLE.getDefaultState());
		}
	...
	public static void setState(boolean active, World worldIn, BlockPos pos) {
		...
		if(active) worldIn.setBlockState(pos, BlockInit.DRAWBRIDGE.getDefaultState().withProperty(FACING, state.getValue(FACING)));
		else worldIn.setBlockState(pos, BlockInit.DRAWBRIDGE.getDefaultState().withProperty(FACING, state.getValue(FACING)));
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		TileEntityDrawbridge tile = (TileEntityDrawbridge) worldIn.getTileEntity(pos);
